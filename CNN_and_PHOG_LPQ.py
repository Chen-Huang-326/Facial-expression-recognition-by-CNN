'''
This file implement a CNN (convolutional neural network) + FC to classify the facial emotions
1.1 The CNN part processes the data of images
1.2 The FC part process the data of LPQ and PHOG features
2.1 Concatenate the learning vectors from CNN and FC to a combined vector
2.2 Use the combined vector to be fed into the last FC layer for classification
'''
...
# import and preprocess PHOG+LPQ data
p = Preprocessor()

'''
The raw SFEW and SFEW.xlsx have the same order of the data,
so we can use the same mask to partition the train/test dataset
'''
# partition the image data into train dataset and test dataset with PPI
...
# partition the LPQ+PHOG data into train dataset and test dataset with PPI (same mask with image data)
train_LP, test_LP = p.PPI_partition(p.data, msk)
# discard the target as it duplicates with the label in image data
# 1~5: LPQ, 6-10: PHOG
train_LP = train_LP.iloc[:,1:]
test_LP = test_LP.iloc[:,1:]

# convert train/test data to tensor
train_dataset = torch.utils.data.TensorDataset(torch.from_numpy(train_data).float(),
                                               torch.tensor(train_LP.values).float(), torch.from_numpy(train_target))
test_dataset = torch.utils.data.TensorDataset(torch.from_numpy(test_data).float(),
                                              torch.tensor(test_LP.values).float(), torch.from_numpy(test_target))
...
def train(epoch):
    ...
    correct = 0
    train_loss = 0
    for step, (batch_x, batch_lp, batch_y) in enumerate(train_loader):
        if args.cuda:
            batch_x, batch_lp, batch_y = batch_x.cuda(), batch_lp.cuda(), batch_y.cuda()
        # clear gradient
        optimizer.zero_grad()
        output = model(batch_x, batch_lp)
        # calculate the loss
...
def test():
    ...
    all_pred = []
    all_target = []
    for batch_x, batch_lp, batch_y in test_loader:
        if args.cuda:
            batch_x, batch_lp, batch_y = batch_x.cuda(), batch_lp.cuda(), batch_y.cuda()
        output = model(batch_x, batch_lp)
